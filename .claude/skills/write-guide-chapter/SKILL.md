...
Example: if `content/fund/chapters/` currently goes up to `19-macro-closeup.html`,
a new chapter there is `20-whatever.html`, even if it should be read second —
you'd place `"20-whatever.html"` as the second entry in the `files` array,
...
- `data-g` groups chapters under a shared heading in the sidebar. **Reuse an
  ...
  for `data-g="` to see current groups (fund: 總覽/器材準備/曝光原理/構圖思維/攝影眼/
  主題實戰; griv: 入門/攝影原理/操作核心/影像風格/進階功能/拍完之後/配件與電力/速查;
  pocket4pro: 基礎認識/基礎設定/常見拍攝手法/進階設定/運鏡技巧/主題運鏡/配件與注意事項;
  advanced: 總覽/調色基礎/調色進階/剪輯入門/剪輯進階). Only introduce a
