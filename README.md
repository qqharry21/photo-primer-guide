...
不用手動操作,網站會自動記住「每個章節捲到哪個小節」,以及全站最後讀到
的那一個:

- 捲動時 debounce 記錄目前讀到的 `<h3>` 小節,重新打開同一章節會自動跳回
  那裡。
- 完全沒帶 hash 冷啟動網站(例如直接打開網址,不是分享連結)時,會直接
  接到上次讀到的那個分頁、那個章節、那個小節,並跳出一則小提示告知。
- 跟收藏功能共用同一組 Token + Gist(見上一節):有設定跨裝置同步的話,
  進度會多存一個 `progress.json` 檔案一起同步;沒設定的話,進度一樣會
  永久留在當下這台裝置的 `localStorage`,不會遺失,只是不會跨裝置。
- 合併策略是「每個章節各自比時間戳,新的贏」——不像收藏功能需要處理
  「刪除」,進度只會被覆寫,所以不需要墓碑機制。

---

## 十、其他功能

- **章節開頭的重點摘要**:每一章開頭都有一個「重點摘要」框(`<div
  class="box tldr">`),3-5 條列出這章的結論或做法,不用整章讀完也能先
  抓到重點——新章節動筆時的規則見
  `.claude/skills/write-guide-chapter/SKILL.md`。
...
| 檔案/資料夾 | 用途 | 可否刪除 |
|---|---|---|
| `index.html` | 殼層:側邊欄、搜尋、日夜模式、收藏面板、閱讀進度、PWA 註冊、多分頁載入引擎 | 否 |
